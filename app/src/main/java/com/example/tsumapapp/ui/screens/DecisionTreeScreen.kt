...
fun informationGain(
    rows: List<DataRow>,
    attribute: String
): Double {//берем список строк данных и название признака по которому бдуем проверять например budget


    val totalEntropy =
        entropy(rows.map { it.label })//rows.map { it.label } — берём только метки из всех строк
    ...
    val weightedEntropy =
        groups.values.sumOf { group -> //доля сия группы от всех данных большие группы влияют сильнее
...
): Treenode //rows текущие строки данных атрибуты не шо не юзали иии
//возвращаем узел дерева Question или Leaf
{
    ...
    val bestAttribute =
        attributes.maxByOrNull { attribute ->//для каждого признака считаем информационный выгрыш и выбираем максимальный
            informationGain(rows, attribute)
        }!!

    //выкидываем использованный признак
    val remaininAttributes =
        attributes.filter { it != bestAttribute }//ставляем только те признаки которые не равны лучшему
    //использованный признак больше не нужен — мы уже разделили по нему

    //Рекурсивно строим
    val groups = rows.groupBy {
        it.attrributes[bestAttribute] ?: ""
    }//строки разбитые по значению лучшего признака спартанцы кароч
    val children = groups.mapValues { (_, groupRows) ->
        buildTree(groupRows, remaininAttributes)//и тепреь для каждой группы строим поддерево
    ...
    //ФИНАЛУС
    return Treenode.Question(
        bestAttribute,
        children
    )//возвращаем узел-вопрос с лучшим признаком и всеми поддеревьями
}

//🚜Часть юая перепишем дерево в список строк для визуала
//depth - глубина узлища, нужна для отступов
fun treeToLines(node: Treenode, prefix: String = "", isLast: Boolean = true): List<String> {
    val lines = mutableListOf<String>()

    when (node) {
        is Treenode.Leaf -> {
            lines.add("$prefix${"└─"}🍽 ${node.recommendation}")
        }

        is Treenode.Question -> {
            lines.add("$prefix${"└─ "}📂 ${node.attributes}?")

            val entries = node.children.entries.toList()
            entries.forEachIndexed { index, (answer, child) ->
                val childPrefix = "$prefix${if (isLast) "   " else "│  "}"
                //показываем ответ
                lines.add("$childPrefix${if (index == entries.size - 1) "└─ " else "├─ "}📌 $answer →")
                //рекурсивно выводим поддерево
                val isLastChild = (index == entries.size - 1)
                val subLines = treeToLines(child, "$childPrefix    ", isLastChild)
...
fun predict(
    ...
    return when (node) {
        //проверяем тип узла
        is Treenode.Leaf -> Pair(
            node.recommendation,
            path
        )//дошли до листа — нашли ответ вернули пару
        //это условие остановки рекурсии

        is Treenode.Question -> {//
            val userAnswer =
                userInput[node.attributes]//что пользователь выбрал для текющего признака например бюджетаа
            val nextNode =
                node.children[userAnswer]//ищем в трентусу ветку которая соответствует ответу пользователя
            ...
            if (nextNode == null) {
                val firstChild =
                    node.children.values.first()//берем первую ветку как запасной вариант
                val newPath =
                    path + "${node.attributes} = $userAnswer (Шота нету:( )"//Добавим шаг в путь шо не нашли
                predict(firstChild, userInput, newPath)//рекурсия с запасной веткой
            } else {//НАШЛИ
                val newPath =
                    path + "${node.attributes} = $userAnswer"//добавляем шаг в путь наприемр "budget = low"
                predict(
                    nextNode,
                    userInput,
                    newPath
                )//рекккккурсия идем в следующий узел с обновлённым путем
            }
        }
    }
}
